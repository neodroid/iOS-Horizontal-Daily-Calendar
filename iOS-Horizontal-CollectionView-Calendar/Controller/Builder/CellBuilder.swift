...
public class CellBuilder {
    ...
    public static func getDateCell(collectionView: UICollectionView, indexPath: IndexPath, startDay: Int) -> UICollectionViewCell{
        
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "dateCell", for: indexPath) as? DateCell {
            cell.setup(title: (indexPath.row + startDay - 1) % 7 , subtitle: "\(indexPath.row + 1)")
            return cell
        }else {
